...
export default class Referee {
	constructor(private state: IStateBoard) {}

	public goodPawn(start: number, end: number, squares: PieceType[], passant_pos=null) {
	...
	public isMoveInvalid(start: number, end: number, squares: PieceType[], passant_pos=null) {
		...
		invalid =
			pawn === true &&
			this.goodPawn(start, end, copy_squares, passant_pos) === false;
		if (invalid) return invalid;
		var king = copy_squares[start].id?.toLowerCase() === "k";
		if (king && Math.abs(end - start) === 2)
			invalid = this.isCastlingPossible(start, end, copy_squares) === false;

		return invalid;
	}

		isCastlingPossible(start: number, end: number, squares: PieceType[]) {
			const copy_squares = squares.slice();
		...
		blockers_exist(start: number, end: number, squares: PieceType[]) {
			var start_row = 8 - Math.floor(start / 8);
			var start_col = (start % 8) + 1;
			var end_row = 8 - Math.floor(end / 8);
			var end_col = (end % 8) + 1;
	...
	pieceCanMoveThere(start: number, end: number, squares: PieceType[], passant_pos = null) {
		const copy_squares = squares.slice();
		if (start === end)
			return false;
		...
		if (this.isMoveInvalid(start, end, copy_squares, passant_pos) === true)
			return false;

		var cant_castle =
			copy_squares[start].id === (player === "w" ? "k" : "K") &&
			Math.abs(end - start) === 2 &&
			this.inCheck(player as "w" | "b", copy_squares);
		...
		if (
			...
			if (this.inCheck(player as "w" | "b", test_squares)) return false;
		}
		...
		if (this.inCheck(player as "w" | "b", check_squares) === true) return false;

		return true;
	}	

	public stalemate(player: "w" | "b", squares: PieceType[]) {
		if (this.inCheck(player, squares)) return false;

		for (let i = 0; i < 64; i++) {
			if (squares[i].player === player) {
					for (let j = 0; j < 64; j++) {
						if (this.pieceCanMoveThere(i, j, squares)) return false;
					}
			}
		}
		return true;
	...
	public checkmate(player: "w" | "b", squares: PieceType[]) {
		if (!this.inCheck(player, squares)) return false;
		for (let i = 0; i < 64; i++) {
			if (squares[i].player === player) {
					for (let j = 0; j < 64; j++) {
	...
	public inCheck(player: "w" | "b", squares: PieceType[]) {
		...
		for (let i = 0; i < 64; i++) {
			if (copy_squares[i].player !== player) {
					if (
						...
						this.isMoveInvalid(i, position_of_king, copy_squares) === false
					)
						return true;
			}
		}
